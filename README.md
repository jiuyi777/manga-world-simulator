...
[本次发布：独立应用 v0.2.42 / 酒馆版 v0.2.77](https://github.com/jiuyi777/manga-world-simulator/releases/tag/v0.2.42)。独立应用新增用户设定，酒馆版保持 v0.2.77。
...
顶部「用户设定」可编辑姓名和人设描述，随当前存档保存、重开恢复及导出导入。姓名用于 {{user}}，人设在每轮正文与开场生成时发送，模型不能擅自覆盖。生成期间或存在待恢复回合时先完成/放弃该回合，再修改设定。

独立应用 v0.2.42 也包含「显示样式 → 正文大小」调节（12–28px，记忆与恢复默认）。
